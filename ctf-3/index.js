...
function authorize(req, res, next) {
  ...
  const id = Array.isArray(req.query.id) ? req.query.id.map(Number) : [Number(req.query.id)]; // Convert query parameter to an array of integers
...
app.get('/cardData/users', authorize , (req, res) => {
  const ids = Array.isArray(req.query.id) ? req.query.id.map(Number) : [Number(req.query.id)]; // Convert query parameter to an array of integers
  console.log("Route ids are:", ids);
  console.log(req.query.id);
  const uid=[parseInt(ids)];
  console.log('Actual:'+ uid);

  const users = cardData.find(u => uid.includes(u.id));
